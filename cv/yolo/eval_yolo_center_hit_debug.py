...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="每行一个图片路径（建议用绝对路径）。",
    )
    parser.add_argument(
        "--test-dir",
        type=str,
        default=None,
        help="如果不提供 image_list，可使用 test-dir 递归扫描图片。",
    )
    parser.add_argument(
        "--labels-root",
        type=str,
        required=True,
        help="label txt 根目录，文件名需与图片同名。",
    )
    parser.add_argument(
        "--weights",
        type=str,
        required=True,
        help="YOLO 权重路径（例如 runs/train/exp/weights/best.pt）。",
    )
    parser.add_argument("--class-id", type=int, default=0, help="评测的目标类别 ID（默认 0）")
    parser.add_argument(
        "--topk",
        type=int,
        default=1,
        help="对目标类别按置信度取前 K 个 box，只要任意一个中心命中 GT 即视为成功（默认 1）。",
    )
    ...
    if args.image_list:
        ...
        images = debug_load_list(list_path)
    elif args.test_dir:
    ...
    if not images:
        print("[ERROR] 没有任何图片可供评测，请检查 image_list 内容或路径。")
        return

    # 简单统计：有多少图片有对应 label
    print("[DEBUG] 开始检查每张图片是否有对应 label...")
    num_have_label = 0
    ...
    for idx, img_path in enumerate(images):
        ...
        # 默认值
        pred_center: Optional[Tuple[float, float]] = None   # top-1 中心
        pred_xyxy: Optional[Tuple[float, float, float, float]] = None  # top-1 bbox
        pred_conf_val: Optional[float] = None               # top-1 置信度
        hit_center: Optional[Tuple[float, float]] = None    # top-K 中真正命中的那个中心
        status = "pred_ok"
        success = False
        ...
        if gt_xyxy is None:
            ...
            if args.save_vis and vis_root is not None:
                save_dir = vis_root / "no_label"
                save_path = save_dir / f"{img_path.stem}.jpg"
                draw_vis(img_path, save_path, None, pred_xyxy, pred_center)
            ...
            rows.append(
                {
                    ...
                    else f"{pred_center[0]:.1f},{pred_center[1]:.1f}",
                    "pred_xyxy": "NA"
                    if pred_xyxy is None
                    else f"{pred_xyxy[0]:.1f},{pred_xyxy[1]:.1f},{pred_xyxy[2]:.1f},{pred_xyxy[3]:.1f}",
        ...
        if (
            ...
            if not mask.any():
                ...
                # Top-K 命中判断：只要前 K 个候选中心有一个在 GT 内，就算成功
                x1, y1, x2, y2 = gt_xyxy
        ...
        # 可视化保存：优先画真正命中的那个中心，否则画 top-1
        if args.save_vis and vis_root is not None:
            if status == "no_pred":
                subdir = "no_pred"
            else:
                subdir = "success" if success else "fail"
            save_dir = vis_root / subdir
            save_path = save_dir / f"{img_path.stem}.jpg"
            vis_center = hit_center if hit_center is not None else pred_center
            draw_vis(img_path, save_path, gt_xyxy, pred_xyxy, vis_center)
        ...
        rows.append(
            {
                ...
                else f"{hit_center[0]:.1f},{hit_center[1]:.1f}",
            }
        )

    # ---- 汇总与输出 ----
    if n_with_gt > 0:
        success_rate = (n_success / n_with_gt) * 100.0
    else:
        success_rate = 0.0
