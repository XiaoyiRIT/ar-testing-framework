...
# -*- coding: utf-8 -*-
"""
v2_evaluation_simple.py — Simple Frame Difference Evaluation Script

与v2_evaluation.py的区别：
- 使用简单的帧差异验证器（simple_diff）
- 不使用复杂的光流法和运动分析
- 只检测"前后两帧是否有变化"
- 避免了复杂的参数调整

验证策略：
1. 全局检测：比较整个画面的像素变化（默认1%）
2. 局部检测：如果有bbox，检测该区域的变化（默认15%）
3. 只要任一检测通过，就认为操作成功

Ground Truth 定义：
- True Positive (TP): CV检测到变化 AND logcat确实记录了操作
- True Negative (TN): CV未检测到变化 AND logcat也没记录
- False Positive (FP): CV检测到变化 BUT logcat没记录
- False Negative (FN): CV未检测到变化 BUT logcat有记录（CV漏检）
"""
...
# 使用简单的帧差异验证器
from src.verifier.backends.simple_diff import verify_action
            ...
            if enable_verify and det is not None and not is_negative:
                ...
                if act_name in cv_verifiable_ops:
                    ...
                    ok = verify_action(
                        ...
                        pre_act,
                        post_act,
                        center_xy=(cx_img, cy_img),
                        bbox=(x_img, y_img, w_img_box, h_img_box),
                        extra={
                            "global_min_change": 0.01,   # 全局变化：1%像素变化
                            "local_min_change": 0.15,    # 局部变化：15%像素变化（在bbox区域）
